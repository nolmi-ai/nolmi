...
- **Repo öffentlich — Open-Source-Richtung GESETZT (Tag 33).** Code wird sichtbar (Open Source), **nicht** nur-Images. Begründung: A2A-Bridge soll Standard + Community werden, Managed ist Bequemlichkeit kein Burggraben, Beiträge + Reichweite gewünscht — alle drei verlangen offenen Code; der Managed-Schutz läuft über die **Lizenz** (§5b AGPL-3.0), nicht übers Verstecken. **§5a-Gate erfüllt** (Scan 🟢 sauber). Reihenfolge: LICENSE auf AGPL-3.0 setzen → Hygiene-Re-Scan → public.
- **Lizenz — GESETZT: AGPL-3.0** (Network-Use-Copyleft §13, schließt die SaaS-Lücke; s. **§5b**). *(MIT als Default verworfen — schlösse die spätere geschützte Option für immer.)* Altlast: die committete Apache-2.0-LICENSE ist vor Public auf AGPL-3.0 zu ersetzen (eigenes BACKLOG-Item).
...
**(b) Lizenz — GESETZT: AGPL-3.0 (Tag 33, recherche-basiert).** Nolmi wird unter **AGPL-3.0** (GNU Affero General Public License v3) lizenziert. Begründung: AGPL ist Copyleft **mit Network-Use-Klausel (§13)** — sie schließt die **SaaS-Lücke** der normalen GPL: wer Nolmi als gehosteten Dienst über das Netzwerk anbietet, muss den (auch modifizierten) Quellcode herausgeben. Das **schützt gegen geschlossene kommerzielle Forks / Konkurrenz-Managed** bei **vollem offenem Code** für Community und Beiträge. Es ist der **etablierte 2026-Standard für Open-Source-SaaS dieser Klasse** (Grafana, Bitwarden-Server, Mattermost, Gitea, Nextcloud, Mastodon, Plausible).

**Warum AGPL-Start (der entscheidende Hebel — Relizenzierungs-Richtung):** Relizenzierung geht nur sicher **restriktiv→permissiv**. **AGPL→MIT** ist jederzeit möglich (der Rechteinhaber lockert); **MIT→AGPL** ist praktisch unmöglich — **jede einmal unter MIT veröffentlichte Version bleibt für immer permissiv forkbar**. „Weg 3 (geschützt) jetzt, Weg 1 (permissiv) langfristig" funktioniert deshalb **nur mit restriktivem Start**: MIT jetzt würde die geschützte Option **für immer schließen**, AGPL jetzt hält **beide** Türen offen.
...
**Bewusst SPÄTER (offene Items, jetzt NICHT entscheiden):**
- **Dual-Licensing** (AGPL frei **+** kommerzielle Lizenz): der **Monetarisierungs-Pfad** für die spätere Managed-Tür — wer Nolmi proprietär/closed betreiben will, kauft eine kommerzielle Lizenz statt unter AGPL offenzulegen. **Ausgestaltung erst, wenn die Managed-Tür konkret wird.** Bei echtem Geld: Lizenztext + **Fachkundige(r) für Lizenzrecht**.
- **CLA/DCO** (Contributor License Agreement / Developer Certificate of Origin): **Vorbedingung für Dual-Licensing** — nötig, damit externe Beiträge kommerziell relizenzierbar bleiben (sonst kann der Rechteinhaber das Gesamtwerk nicht dual-lizenzieren). Muss **vor dem ersten externen Beitrag** stehen. Jetzt unkritisch (Alleinautor), aber als **Gate vor „erste Beiträge annehmen"** gemerkt.

**Altlast (jetzt-relevant, vor Going Public zu fixen):** Das Repo trägt aus #111 (Repo-Hygiene Tag 25) noch eine **Apache-2.0-`LICENSE` + `package.json: "license": "Apache-2.0"`** — beides **widerspricht** der AGPL-Setzung. **Vor Public:** `LICENSE` auf **AGPL-3.0**-Volltext ersetzen, `package.json: "license": "AGPL-3.0"`, ggf. Header/README angleichen. **Nicht in diesem Doku-Commit** — eigener Schritt im Going-Public-Block (BACKLOG-Item).
