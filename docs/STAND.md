...
- **Lizenz-Default MIT → Copyleft-Start (AGPL-artig) revidiert.** Entscheidender Hebel: Relizenzierung geht nur sicher **restriktiv→permissiv** (AGPL→MIT jederzeit; MIT→AGPL praktisch unmöglich — jede MIT-Version bleibt für immer forkbar). „Geschützter Start, permissiv-nah langfristig" funktioniert **nur mit restriktivem Start**; MIT jetzt schlösse die spätere Option für immer. Finale Wahl **offen am Lizenztext** (ggf. mit Fachkundigem), nicht aus dem Stand fixiert. **Altlast:** die Apache-2.0-`LICENSE` + `package.json` aus #111 sind relativ zur neuen Richtung veraltet → vor Public anzupassen (hier bewusst noch nicht — reine Strategie-Setzung).
...
- **Offen (Folge-Check, kein Blocker):** Advanced-Pfad (volle PersonaInput, Mandate-Wahl, **eigene Bridge** via Register-Token — braucht laufende Bridge). **TTY-Befund:** `readLine`/`readSecret` teilen keinen Buffer über Aufrufe → nur interaktiv, nicht piped/CI (Helper-Refactor wäre separates Stück). Beides als BACKLOG-Item notiert.
