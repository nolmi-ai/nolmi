...
**Phase 4.3 Stand:** Schritte 1 (Schema) · 2 (Ton) · 3 (Sichtbarkeit+Kontrolle) · 5 (Autonomie) gebaut + lokal verifiziert, NICHT deployt. Schritt 4 (Auto-Ableitung) daten-vertagt. **Funktional vollständig** (bis auf das datenabhängige Schritt 4): Twin kennt Vertrautheit, passt Ton an, Owner kontrolliert manuell, Autonomie level-basiert + konservativ. **Deploy: gebündelt auf Signal** — „mit-Migration"-Klasse (029) → WAL-DB-Backup vor Deploy wie 028. Deploy-Verify-Punkte: Browser-Check Schritt-3-Selektor (steht noch aus), A2A-Ton-Effekt (Schritt 2) am echten Verkehr, Graded-Mitte-Dispatch (Schritt 5) live.
