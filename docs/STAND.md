...
- **VPS-Schritt (separat, nach diesem Commit):** `git pull` + `docker compose up -d --remove-orphans` aus `docker/nolmi/` entfernt den laufenden `nolmi-apex`-Container (Traefik/Produktion unberührt). Reversibel (A-Record zurück auf VPS → Diff zurücknehmen).
