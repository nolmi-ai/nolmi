...
**Letztes Update:** 29. Mai 2026, Freitag (Tag 31 Block 10 — B2-Prep: Nolmi-Stack-Compose unter docker/nolmi/ autoret, 3 Services inkl. Bridge, external-Netz, Staging-ACME-Default, BasicAuth same-bring-up; `compose config` grün, noch nicht deployed)
...
**Tag-31-Total bis Block 10:** 10 Blöcke + 1 operative Mini-Closure (getnolmi.com-Redirect), ~3.5h Netto + Phase-4-Strategy-Session + B3-Pre-Flight-Diagnose + S2-Korrektur + Bau-Block B1 + B2-Prep. Rebrand-Pipeline Phase 1+2+3a+3b ✅ live im Repo + Hygiene-Pass abgeschlossen + Phase-4-Bau-Vorlage gelockt (`docs/PHASE-4-VPS-STRATEGY.md`, 7 Setzungen, 3 hängende Items aufgelöst) + B3 Pre-Flight verifiziert + S2 final auf Doppel-DB-Migration korrigiert + **B1 done** (VPS-Prep + Docker + Traefik v3.6 auf `187.124.3.235`, verifiziert) + **B2-Prep** (Nolmi-Stack-Compose unter `docker/nolmi/` autoret, `compose config` grün, noch nicht deployed). Drei echte Twins in lokaler DB. Code-Rebrand-Teil + Hygiene vollständig, Phase 4 läuft — **nächster Schritt B2-Deploy-Runbook** (Dockerfile-Filter auf `@nolmi/*`, Images bauen, Traefik le-staging-Resolver, htpasswd, leere Volumes hochfahren).
