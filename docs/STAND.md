...
**Letztes Update:** 30. Mai 2026, Samstag (Tag-31-Push fortgesetzt, Block 22 — Distribution Etappe 2.1: `twin:bootstrap` setzt `owner_user_id` jetzt via `OWNER_EMAIL`-Lookup (`UsersRepo.findByEmail()`), Fallback `OWNER_USER_ID`, `WARN` bei fehlendem Owner statt stillem NULL. **Release-Blocker Self-Hosting behoben** — frischer Solo-Twin erscheint sofort im owner-gescopten Switcher. Lokal am Verhalten verifiziert (Wegwerf-Twin @solo2: owner gesetzt in DB, Switcher-Query liefert ihn, WARN-Gegenprobe, Fehler-Pfad). Vorher Block 21 — Distribution Etappe 1 lokal am Verhalten verifiziert (Solo-Twin @solo: 4/4 grün))
...
**Tag-31-Total bis Block 22:** 22 Blöcke + 1 operative Mini-Closure (getnolmi.com-Redirect), ~3.5h Netto + Phase-4 (Strategy→B1–B6, Nolmi produktiv) + ROADMAP-Re-Baseline + Distribution-Strategie-Session + Distribution Etappe 1 (Bridge-Optionalität, gebaut + interaktiv am Verhalten verifiziert) + Distribution Etappe 2.1 (`owner_user_id`-Bootstrap-Fix, Release-Blocker behoben). **Distribution-Stand:** Etappe 0 (Diagnosen) ✅ + Etappe 1 (Bridge optional) ✅ lokal verifiziert (4/4) + Etappe 2.1 (`owner_user_id` via `OWNER_EMAIL`) ✅ lokal verifiziert (Release-Blocker weg); nächster: Etappe 2.2 (CLI-Onboarding koppelt User-Anlage + Owner-Zuweisung).
