...
- **Verifikations-Vorbehalt (bewusst):** Belege nutzen einen deterministischen LLM-Mock — der `subject`-Plumbing-Pfad ist real bewiesen, die introspektive **Text-Qualität** des echten LLM-Calls steht aus (erster realer `twin:reflect`-Lauf zeigt, ob der Prompt ohne Mock trägt). **Zurückgestellt (notiert):** dedizierter `'reflection'`-Diary-Enum-Wert (🟡, mit nächster Diary-Migration bündeln); Background-Loop-Trigger `'scheduled'` = Stufe 2; Frequenz-Dimension bei Zeit-Erleben = Stufe 2 (episodic-only, schemafrei).
