...
**Letztes Update:** 29. Mai 2026, Freitag (Tag 31 Block 14 — B2 VOLLSTÄNDIG ABGESCHLOSSEN: Prod-Cert-Flip auf le, Issuer „Let's Encrypt CN=YR2" über alle 3 Hosts, TLS-verify=0, app→401; Flip griff erst nach Resolver-Cache-Fix (§7 B2-4). Nolmi-Stack läuft end-to-end auf echter Infra mit vertrauten Certs — Wegwerf-Daten, Production unberührt. Nächster Block B4)
...
**Tag-31-Total bis Block 14:** 14 Blöcke + 1 operative Mini-Closure (getnolmi.com-Redirect), ~3.5h Netto + Phase-4-Strategy-Session + B3-Pre-Flight-Diagnose + S2-Korrektur + Bau-Block B1 + B2-Prep + Phase-3a-Dockerfile-Nachzügler + Bridge-Auto-init + B2 (Staging→Prod-Certs). **Phase-4-Stand:** B1 ✅ + B2 ✅ (Prod-Certs, vertraut) + B3 ✅ Pre-Flight; verbleibend B4 (Doppel-DB-Migration) + B5/B6 (Smoke + Cut-Over) + B7 (Abschaltung). Rebrand-Pipeline Phase 1+2+3a+3b ✅ live im Repo + Hygiene-Pass abgeschlossen + Phase-4-Bau-Vorlage gelockt (`docs/PHASE-4-VPS-STRATEGY.md`, 7 Setzungen, 3 hängende Items aufgelöst) + B3 Pre-Flight verifiziert + S2 final auf Doppel-DB-Migration korrigiert + **B1 done** (VPS-Prep + Docker + Traefik v3.6 auf `187.124.3.235`, verifiziert) + **B2-Prep** (Nolmi-Stack-Compose unter `docker/nolmi/` autoret, `compose config` grün, noch nicht deployed). Drei echte Twins in lokaler DB. Code-Rebrand-Teil + Hygiene vollständig, Phase 4 läuft — B1 ✅ + B2 ✅ (Prod-Certs, vertraut) + B3 ✅. **Nächster Block: B4** (Doppel-DB-Migration `twin.db` + `bridge.db`, gemeinsamer Freeze-Snapshot, echter Encryption-Key vom alten VPS).
