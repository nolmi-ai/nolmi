...
**B2-Befund 4 (MITTEL/HART) — Resolver-Wechsel zieht keine neuen Certs, solange die Domain im alten Resolver-Store liegt:** Beim Staging→Prod-Flip (`ACME_RESOLVER` `le-staging` → `le`) lieferte Traefik **weiter die Staging-Certs**, obwohl das `tls.certresolver`-Label korrekt auf `le` stand und der Prod-`acme.json` leer war. Traefik matcht vorhandene Certs primär nach **Domain**, nicht nach Resolver — fand `app/runtime/bridge.nolmi.ai` im `acme-staging.json` und sah keinen Bezugsbedarf. **Symptom:** `TLS-verify=20` (untrusted), HTTP `000`, ACME-Log leer (kein `le`-Bezugsversuch). **FIX (VPS verifiziert):** `acme-staging.json` leeren (`> file` + `chmod 600`) + Traefik-Restart + einen Request pro Host als Bezugs-Trigger → Prod-Certs in ~30–90 s gezogen. **Final-Verify:** Issuer `Let's Encrypt CN=YR2` (kein STAGING) über alle drei Hosts, `TLS-verify=0`.

**NACHSCHÄRFUNG (3b-Automatisierung, Wegwerf-VM `test.nolmi.ai`, Tag 33) — „Staging-Store leeren" allein reicht NICHT, BEIDE Stores müssen leer:** Beim automatisierten Flip (`install/tls-promote.sh`) zeigte sich der Befund tiefer als oben. Nur `acme-staging.json` zu leeren ließ Traefik **weiter das Staging-Cert servieren**: der `le`-Prod-Resolver registrierte zwar einen **Account** (`acme.json` = Account-only, **keine** Certs), bezog aber nichts — weil der **neu gefüllte** Staging-Store (Restart → in-memory) für die Domains „kein Bezugsbedarf" signalisierte. Label `le`, Prod-`caserver`, `.env` — alles korrekt verdrahtet, der Bezug fand trotzdem nicht statt. **FIX (manuell verifiziert, alle 3 Hosts → echtes Let's Encrypt `CN=YR1/YR2`, `TLS-verify=0`):** **`acme-staging.json` UND `acme.json` leeren** (`echo -n > file` + `chmod 600` beide) → **dann** Traefik-Restart (lädt leere Stores → kein Cert zum Matchen) → **dann** Trigger (1 Request/Host) → Neu-Bezug gegen `le`. **Trade-off:** `acme.json` mitzuleeren wirft den Prod-ACME-Account weg → bei jedem Flip neuer Account + frischer Bezug; für den **einmaligen** Flip unkritisch (Account-Anlage gratis, kein Rate-Limit), bei wiederholtem Flip bedenken. Die manuelle Phase-4-Pause (Block 14) hat den vollen Store-Reset zufällig vermieden und den Befund daher verfehlt.

**ZWEITE NACHSCHÄRFUNG (VM-Gegentest, Tag 33) — REIHENFOLGE: le-Label am Stack aktiv BEVOR die Stores geleert werden:** Beide Stores zu leeren reichte im Skript-Gegentest noch **nicht** — der Lauf zog wieder Staging (`acme.json` leer, `acme-staging.json` voll, alle Hosts STAGING), obwohl `.env`/Label korrekt auf `le`. Ursache: ein **Zeitfenster**. Wenn die Stores geleert werden, **bevor** der Stack auf das `le`-Label recreatet ist, läuft Traefik kurz mit leeren Stores **und** noch dem alten `le-staging`-Label → der Neu-Bezug in diesem Fenster geht gegen Staging. Der manuelle Fix klappte nur, weil der Stack dort schon mit `le`-Label lief, bevor die Stores geleert wurden. **FIX (VM-verifiziert, alle 3 Hosts → echtes Let's Encrypt `CN=YR1/YR2`, `TLS-verify=0`, `acme.json` 40971 Bytes = Certs, BasicAuth intakt):** **Stack-Recreate (`le`-Label) ZUERST → dann beide Stores leeren + Traefik-Restart → dann Trigger.** Vollständige Reihenfolge: **(1) `.env`→`le` (2) Stack-Recreate (3) beide Stores leeren + Restart (4) Trigger/Verify.** `le` muss am Stack durchgängig aktiv sein, bevor der erste Bezug ausgelöst wird.

**DRITTE NACHSCHÄRFUNG (Verify-Robustheit) — „TRAEFIK DEFAULT CERT" ist KEIN Erfolg:** Die Verify-Schleife wertete vor abgeschlossenem Bezug Traefiks **Platzhalter-Cert** (`CN=TRAEFIK DEFAULT CERT`) fälschlich als „PROD-Issuer". Ein echtes Prod-Cert erfüllt **alle drei**: enthält `Let's Encrypt`, **nicht** `STAGING`, **nicht** `TRAEFIK DEFAULT CERT`. `tls-promote.sh` prüft das jetzt in einer **Retry-Schleife** (6×15 s ≈ 90 s) und meldet erst dann grün — sonst eine ehrliche Timeout-Warnung mit manuellem Prüf-Befehl. So kann Skript-grün dem Self-Hoster kein Default-/Staging-Cert als Erfolg verkaufen.

**Cookbook-Konsequenz:** Wer eine Staging-zuerst-Strategie fährt (empfohlen, schützt die Prod-Rate-Limits), muss beim Flip (a) das **`le`-Label am Stack aktivieren** (Recreate), (b) **BEIDE ACME-Stores leeren** (Staging **und** Prod) + Traefik-Restart, (c) triggern — **in dieser Reihenfolge** (Label-aktiv vor Store-Reset, sonst Staging-Fenster). Alternativ: Staging nur mit separatem Hostnamen testen. §9.3/§9.5-Cookbook sollte den Flip-Schritt **inkl. Reihenfolge + vollem Store-Reset (beide Stores)** dokumentieren — `install/tls-promote.sh` ist die Referenz-Implementierung.
