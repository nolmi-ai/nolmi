...
- **(b) Sprachnachrichten an den Twin (STT):** 🟡 **SS1 ✅ LIVE (Tag 50), SS2–4 offen.** Self-hosted Whisper — **OpenAI-kompatibler Sidecar** `fedirz/faster-whisper-server:latest-cpu`, Modell **`small`** (ENV `WHISPER__MODEL`, double underscore) + 🔴 **`initial_prompt`** (Domänen-Vokabular) + `language=de`. **SS1 (`dfe8a98`/`c967c96`) live + Prod-verifiziert:** Sidecar internal-only (`nolmi-internal`, kein Port), `WHISPER__MODEL=small` greift (Log „Model small loaded in 6.13s"), interner Pfad bewiesen (runtime → `http://nolmi-whisper:8000` → echte Transkription). 🔴 **Befunde:** Lazy-Load (~6s beim 1. Request, offload nach 300s idle → keep-warm-Optimierung später); small ist „gut genug für den Sinn", nicht wortperfekt (kurzes Audio „Whisper"→„Vista" trotz Prompt). 🔴 **Echt-Voice-Vorbehalt OFFEN** — beide Tests synthetisch (espeak); echte Stimme/Telegram-Opus nie getestet → bei SS3 verifizieren. **Architektur:** Audio NICHT nativ ans Modell → Transkription DAVOR; `transcribeAudio(bytes)`-Helfer für beide Eingänge. **OFFEN:** SS2 `transcribeAudio`-Helfer (`POST {WHISPER_URL}/v1/audio/transcriptions`, multipart+model+language+initial_prompt), SS3 Telegram `bot.on("voice")` (OGG/Opus via getFileLink; heute nur text `telegraf-setup.ts:94`) **+ Echt-Voice-Test**, SS4 Web `POST /twins/:handle/transcribe` + UI. Audio = Vorverarbeitung, KEIN Modell-Attachment. Optimierung: Whisper keep-warm (Lazy-Load-Aufschlag). Größe Rest M–L.
