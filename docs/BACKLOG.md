...
- **(b) Sprachnachrichten an den Twin (STT):** 🟡 **Spike ✅ GRÜN (Tag 50), Integration offen.** Self-hosted Whisper machbar — **OpenAI-kompatibler Sidecar** `fedirz/faster-whisper-server:latest-cpu`, Modell **`small`** + 🔴 **`initial_prompt`** (Domänen-Vokabular → wortgenau + Latenz 16.8s→8.2s), `language=de`. VPS-Gate grün (7.8 GB RAM / 2 vCPU EPYC, CPU-only). Vorbehalt: Test war synthetisch (espeak) → echte Voice beim ersten Flow verifizieren. **Architektur:** Audio NICHT nativ ans Modell → Transkription DAVOR (Audio→Whisper→Text→Modell); `transcribeAudio(bytes)`-Helfer für beide Eingänge. **OFFEN (Integration):** Sidecar ins Compose/Override (model-cache-Volume, nolmi-internal), `transcribeAudio`-Helfer, Telegram `bot.on("voice")` (OGG/Opus via getFileLink; heute nur text `telegraf-setup.ts:94`), Web `POST /twins/:handle/transcribe` + UI. Audio = Vorverarbeitung, KEIN Modell-Attachment. Größe M–L.
